...
API endpoint все еще возвращает `{"detail":"Database not configured"}` вместо данных вайфу.
...
### 4. Обновление API сервера

Давайте обновим API сервер для работы с базой данных:
...
3. **Добавьте в Render как `DATABASE_URL`**

### Шаг 2: Настройка Redis

1. **Войдите в Redis Cloud Dashboard**
2. **Скопируйте Connection String**
3. **Добавьте в Render как `REDIS_URL`**
