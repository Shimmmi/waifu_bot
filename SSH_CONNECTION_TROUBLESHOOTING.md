# 🔧 Устранение проблемы "Connection reset" при SSH подключении

## Проблема

```
kex_exchange_identification: read: Connection reset
```

Это означает, что SSH соединение сбрасывается во время установки соединения.

---

## 🔍 Диагностика

### Шаг 1: Проверка доступности сервера

```powershell
# Проверка ping
ping 45.156.21.220

# Проверка порта 22
Test-NetConnection -ComputerName 45.156.21.220 -Port 22
```

### Шаг 2: Проверка SSH подключения с детальными логами

```powershell
ssh -v rocketcloud-vps "echo 'test'"
```

Ищите в выводе:
- `Connection reset` - сервер закрывает соединение
- `Connection refused` - SSH сервис не запущен
- `Connection timed out` - файрвол блокирует
- `Permission denied` - проблема с аутентификацией

### Шаг 3: Проверка SSH конфигурации

```powershell
cat ~/.ssh/config
```

---

## ✅ Решения

### Решение 1: Проверка статуса SSH сервиса на сервере

Если у вас есть доступ к панели управления RocketCloud:

1. Зайдите в панель управления
2. Откройте консоль VPS (web console)
3. Выполните:

```bash
# Проверка статуса SSH
systemctl status ssh
# или
systemctl status sshd

# Если не запущен, запустите:
systemctl start ssh
systemctl enable ssh
```

### Решение 2: Очистка known_hosts

Иногда помогает очистка старых записей:

```powershell
# Удаление записи для этого хоста
ssh-keygen -R 45.156.21.220
ssh-keygen -R rocketcloud-vps
```

### Решение 3: Попробуйте прямое подключение

```powershell
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@45.156.21.220
```

### Решение 4: Проверка через другой порт

Если SSH работает на другом порту:

```powershell
# Попробуйте подключиться с указанием порта
ssh -p 22 root@45.156.21.220
```

### Решение 5: Использование пароля вместо ключа

Если ключ не работает, попробуйте пароль:

```powershell
ssh -o PreferredAuthentications=password -o PubkeyAuthentication=no root@45.156.21.220
```

### Решение 6: Проверка через веб-консоль RocketCloud

1. Зайдите в панель управления RocketCloud
2. Найдите вашу VPS
3. Откройте "Console" или "Web Terminal"
4. Выполните команды для диагностики:

```bash
# Проверка SSH
systemctl status ssh

# Проверка логов SSH
journalctl -u ssh -n 50
# или
tail -50 /var/log/auth.log

# Проверка порта 22
netstat -tlnp | grep :22
# или
ss -tlnp | grep :22

# Проверка файрвола
ufw status
# или
iptables -L -n
```

---

## 🔧 Исправление проблем на сервере

### Если SSH сервис не запущен:

```bash
# Запуск SSH
systemctl start ssh
systemctl enable ssh

# Проверка
systemctl status ssh
```

### Если SSH заблокирован файрволом:

```bash
# Открытие порта 22 (если используется ufw)
ufw allow 22/tcp
ufw reload

# Или для iptables
iptables -A INPUT -p tcp --dport 22 -j ACCEPT
iptables-save
```

### Если слишком много подключений:

```bash
# Проверка активных подключений
ss -tn | grep :22 | wc -l

# Очистка старых соединений (если нужно)
# Обычно это делается автоматически
```

### Если проблема с SSH конфигурацией:

```bash
# Проверка конфигурации SSH
cat /etc/ssh/sshd_config | grep -E "Port|PermitRootLogin|PubkeyAuthentication|PasswordAuthentication"

# Если нужно перезапустить SSH
systemctl restart ssh
```

---

## 🔄 Альтернативные способы подключения

### 1. Использование веб-консоли RocketCloud

- Зайдите в панель управления
- Откройте "Console" или "Web Terminal"
- Выполните команды напрямую

### 2. Использование PuTTY (Windows)

1. Скачайте PuTTY: https://www.putty.org/
2. Заполните:
   - Host Name: `45.156.21.220`
   - Port: `22`
   - Connection type: SSH
3. В Connection → SSH → Auth укажите путь к приватному ключу
4. Подключитесь

### 3. Использование WinSCP

1. Скачайте WinSCP: https://winscp.net/
2. Подключитесь к серверу
3. Используйте встроенный терминал для выполнения команд

---

## 📋 Быстрая диагностика

Выполните эти команды по порядку:

```powershell
# 1. Проверка ping
ping 45.156.21.220

# 2. Проверка порта
Test-NetConnection -ComputerName 45.156.21.220 -Port 22

# 3. Очистка known_hosts
ssh-keygen -R 45.156.21.220

# 4. Попытка подключения с детальными логами
ssh -v root@45.156.21.220 "echo 'test'"

# 5. Попытка с другими параметрами
ssh -o ConnectTimeout=30 -o ServerAliveInterval=30 root@45.156.21.220
```

---

## 💡 Если ничего не помогает

1. **Обратитесь в поддержку RocketCloud:**
   - Опишите проблему
   - Укажите IP: `45.156.21.220`
   - Приложите вывод `ssh -v`

2. **Проверьте через веб-консоль:**
   - Используйте веб-консоль RocketCloud для доступа к серверу
   - Проверьте статус SSH сервиса
   - Проверьте логи

3. **Перезапустите VPS:**
   - Через панель управления RocketCloud
   - Подождите 2-3 минуты
   - Попробуйте подключиться снова

---

## 🔍 Дополнительные команды для диагностики

### На клиенте (Windows):

```powershell
# Проверка SSH конфига
cat ~/.ssh/config

# Проверка ключа
ls -la ~/.ssh/

# Тест подключения с разными параметрами
ssh -v -o ConnectTimeout=10 root@45.156.21.220
ssh -v -o PreferredAuthentications=password root@45.156.21.220
ssh -v -p 2222 root@45.156.21.220
```

### На сервере (через веб-консоль):

```bash
# Проверка SSH
systemctl status ssh
journalctl -u ssh -n 50

# Проверка портов
netstat -tlnp | grep :22
ss -tlnp | grep :22

# Проверка файрвола
ufw status verbose
iptables -L -n -v

# Проверка логов
tail -50 /var/log/auth.log
tail -50 /var/log/syslog | grep ssh
```

